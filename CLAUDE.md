...
[`ask-matt`](./skills/engineering/ask-matt/SKILL.md) is the router that maps every user-reachable skill and how they relate. Keep it honest about the skills it does name: if you rename or remove a skill `ask-matt` routes to, fix that reference in the same change, because a router pointing at a skill that no longer exists is worse than one that stays quiet. Adding a new skill does **not** oblige you to add it to `ask-matt`; do that when the skill changes how an existing flow is entered, or when asked. `ask-matt` is `disable-model-invocation: true` and has never been invoked in this machine's transcript corpus, so a completeness sweep on every skill addition is maintenance with no reader.

To (re)link every skill outside `deprecated/`, `misc/`, and `in-progress/` into the local harness skill directories (`~/.claude/skills`, `~/.agents/skills`), run `scripts/link-skills.sh`. Each entry is a symlink into this repo, so a `git pull` keeps installed skills current; re-run the script after adding, removing, or renaming a skill.

Skipping `in-progress/` is a **local deviation from upstream**, which links that bucket so its feedback loop runs locally. Expect the comment above the `find` call to conflict on rebase. Two reasons for it: none of the bucket's skills has been invoked once in the whole transcript corpus (they are all `disable-model-invocation: true`, so a zero count means never typed), and `in-progress/retro` and `howardism/retro` are different skills sharing a name, which in a flat destination directory made `find` ordering decide which `/retro` the user got.

The linker only adds and replaces, never prunes. Move a skill into a skipped bucket and its old symlinks survive in both destinations, still resolving, so the demoted skill keeps loading. Delete them by hand in the same change. Run it from the main checkout only: run inside a git worktree (e.g. by a subagent) it silently points every symlink at the worktree, and they all dangle when the worktree is removed; re-run from the main checkout after merging.
