...
For an approved plan handed to a worker, recommend **`/implement-with-subagent`**.
For completed changes needing atomic commits, recommend **`/commit-with-subagent`**;
...
- **`/refine-harness`**: prune and relocate the always-on config itself: global CLAUDE.md, rules-engine rules, hooks, auto-memory, skill listing, permissions, mirrors. Evidence first (audit log, transcripts), then Anthropic's guidance, then one move per finding: keep, cut, move, merge, sharpen, harden. Where `/retro` finds config that is missing and `/refine-skill` fixes one skill, this cores through the sediment that is already loaded every session.
- **`/find-skills`**: reaches past the skills you already have: searches the open agent-skills ecosystem for one that does the thing, and installs it. Where this router maps what's installed, find-skills looks outside it.
- **`/codex-refine-harness`**: review global Codex AGENTS.md, diagnose instruction failures, and apply authorized improvements to that file only. Global CLAUDE.md is a read-only cross-reference. Use this for Codex instruction refinement; `/refine-harness` covers the broader Claude harness.
- **`/codex-fewer-permission-prompts`**: mines recent Codex transcripts for the read-only commands you keep approving and proposes narrow prefix rules for them. Harness tuning, not code work.
- **`/perf-review`**: the entry point for "make it faster" / "find perf wins" flows. Fans the perf-* angle skills out as parallel subagents over a target, then merges their findings into ranked, measurable suggestions. Reports by default and ships only when you ask for it: say "create a PR per angle" and each angle becomes its own benchmarked branch. Distinct from `/code-review` (standards/spec, not runtime cost). Its five angle skills: `/perf-measurement` (back-of-envelope costing and benchmarking, the shared reference the others cost their findings against), `/perf-algorithms`, `/perf-memory`, `/perf-api`, `/perf-overhead` are model-invoked and individually reachable when only one angle is in question.
